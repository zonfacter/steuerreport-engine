...
def test_tax_domain_summary_splits_private_business_data_credits_and_derivatives() -> None:
    ...
    assert summary["euer"]["betriebseinnahmen_mining_staking_eur"] == "17.50"
    assert summary["euer"]["betriebsausgaben_data_credits_eur"] == "1.25"
    assert summary["euer"]["betriebsergebnis_eur"] == "16.25"
    assert summary["termingeschaefte"]["netto_eur"] == "-25"
    assert summary["termingeschaefte"]["verlust_summe_abs_eur"] == "40"


def test_tax_domain_summary_routes_business_disposals_to_euer_not_private_so() -> None:
    summary = build_tax_domain_summary(
        raw_events=[],
        tax_lines=[
            {
                ...
                "gain_loss_eur": "-20",
                "tax_status": "business",
                "tax_domain": "euer_business_disposal",
                "lot_domain": "business",
            },
            {
                "asset": "SOL",
    ...
    assert summary["anlage_so"]["private_veraeusserung_net_taxable_eur"] == "50"
    assert summary["euer"]["business_disposal_proceeds_eur"] == "80"
    assert summary["euer"]["business_disposal_cost_basis_eur"] == "100"
    assert summary["euer"]["business_disposal_net_eur"] == "-20"
    assert summary["euer"]["betriebsergebnis_eur"] == "-20"


def test_tax_domain_summary_uses_value_resolver_for_reward_values() -> None:
    raw_events = [
        {
            "unique_event_id": "reward-usd",
            "payload": {
    ...
    summary = build_tax_domain_summary(
        ...
        tax_year=2025,
        ruleset_id="DE-2025-v1.0",
        value_resolver=lambda payload: Decimal("12.34"),
    )

    assert summary["classification_counts"]["unresolved_valuation_events"] == 0
    assert summary["anlage_so"]["leistungen_income_eur"] == "0"
    assert summary["euer"]["betriebseinnahmen_mining_staking_eur"] == "12.34"


def test_tax_domain_summary_uses_heliumgeek_display_quantity() -> None:
    raw_events = [
        {
            "unique_event_id": "heliumgeek-iot",
            "payload": {
    ...
    summary = build_tax_domain_summary(
        ...
        derivative_lines=[],
        tax_year=2024,
        ruleset_id="DE-2024-v1.0",
    )

    assert summary["anlage_so"]["leistungen_income_eur"] == "0"
    assert summary["euer"]["betriebseinnahmen_mining_staking_eur"] == "20.38227210"
