...
@app.get("/api/v1/rulesets", response_model=StandardResponse, tags=["rulesets"])
def ruleset_list(include_pending: bool = True) -> StandardResponse:
    ...
    for _key, row in catalog_index.items():
        if any(item["ruleset_id"] == row["ruleset_id"] and item["ruleset_version"] == row["ruleset_version"] for item in merged):
            continue
...
@app.post("/api/v1/rulesets", response_model=StandardResponse, tags=["rulesets"])
...
@app.get("/api/v1/rulesets/{ruleset_id}/{ruleset_version}", response_model=StandardResponse, tags=["rulesets"])
